...
var reloadMetrics = {
	...
	reloadMetrics: function(taskName)
	{
		return new Promise(function(resolve, reject)
		{
			
			logger.info('reloadMetrics::Starting Task::' + taskName, {module: 'reloadmetrics'});
			var path = "https://" + config.hostname + ":" + config.qrsPort + "/qrs/task/start/synchronous";
			path += "?xrfkey=ABCDEFG123456789&name=" + taskName;
				...
				if(typeof result =='object')
				{
					...
					progressCheck(taskId, function(error, result)
					{
						...
						else
						{
							...
							var path2 = "https://" + config.hostname + ":" + config.qrsPort + "/qrs/executionresult";
							path2 += "?xrfkey=ABCDEFG123456789&filter=executionid eq " + taskId;
...
{
	//console.log(reload);
	var path = "https://" + config.hostname + ":" + config.qrsPort + "/qrs/executionsession";
	path += "?xrfkey=ABCDEFG123456789&filter=id eq " + id;
