	...
	updateMetrics : function(appRef, data)
	{
		return new Promise(function(resolve, reject)
		{
			...
			.then(function(appSubjectAreas)
			{
				...
				return objectMgmt.manageObjects(appId, data, appSubjectAreas)
				.then(function(message)
				{
					logger.info(message, {module: 'updateMetrics'});
					return changeOwner.changeAppObjectOwner(config.qrs.repoAccountUserDirectory, config.qrs.repoAccountUserId, appId)
					.then(function(message)
					{
						logger.info(message, {module: 'updateMetrics'});
						...
						{
							logger.debug(error, {module: 'updateMetrics'});
						});
					})
					.catch(function(error)
					{
