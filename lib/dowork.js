...
var getDocList = require('./getDocListByCustomProperty');
...
var doWork = {
	...
	getAllMdi: function()
	{
		return new Promise(function(resolve, reject)
		{
			...
			// "MetricsLibraryInput"
			getDocList.getDocListByCustomProperty(config.gms.metricLibraryInputPropName)
			.then(function(docList)
			{
				// Step 2: For each Qlik Sense app, get the master data items and
				// deliver return them in the right format for the metrics library
				Promise.map(docList, function(doc)
				{
					var docName = doc.qDocName;
          // Remove the .qvf extension if present
          var metricSubject = (docName.indexOf('qvf') !== -1) ? docName.slice(0, -4) : docName
					return getMdis.getMdis(doc, metricSubject)
				})
				.then(function(result)
				{
