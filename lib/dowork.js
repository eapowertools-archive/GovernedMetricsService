...
var doWork = {
    getDoc: function (body) {
        return new Promise(function (resolve, reject) {
            logger.debug('Calling getDoc', {
                module: 'doWork'
            });
            getdoc.getDocId(body)
                .then(function (doc) {
                    logger.debug('getDoc success', {
                        module: 'doWork'
                    });
                    resolve(doc);
                })
                .catch(function (error) {
                    logger.error('getDoc failure::' + error, {
                        module: 'doWork'
                    });
    ...
    deleteFromApp: function (appname) {
        return new Promise(function (resolve, reject) {
            logger.info('deleteFromApp::Calling deleteFromApp', {
                module: 'doWork'
            });
            var message;
            var x = {};
            ...
            return getdoc.getDocId(appname.appname)
                .then(function (doc) {
                    return deleteMetrics.deleteAllMasterItems(doc)
                        .then(function (result) {
                            ...
                            logger.info('deleteFromApp success', {
                                module: 'doWork'
                            });
                            resolve(res);
                        })
                        .catch(function (error) {
                            logger.error('deleteFromApp failure::' + JSON.stringify(error), {
                                module: 'doWork'
                            });
                            reject(error);
                        });
                ...
                .catch(function (error) {
                    logger.error('deleteFromApp failure::' + error, {
                        module: 'doWork'
                    });
                    reject(new Error(error));
                });
    ...
    addAll: function () {
        logger.info('Calling addAll, which forwards to updateAll', {
            module: 'doWork'
        });
        doWork.updateAll();
    },
    updateAll: function () {
        return new Promise(function (resolve, reject) {
            ...
            logger.info('Checking for existence of gms tag in the qrs', {
                module: 'doWork',
                        ...
                        return qrsInteract.Get(propExistPath)
                            .then(function (customProp) {
                                if (customProp.body !== undefined || customProp.body.length != 0) {
                                    logger.info('Found ' + customProp.body[0].name + ' in list of custom properties', {
                                        module: 'doWork'
                                    });
                                    var x = {};

                                    var y = {};
                                    logger.debug('getMetricsTable', {
                                        module: 'doWork'
                                    });
                                    return gethypercube.getMetricsTable()
                                        ...
                                        .then(function (matrix) {
                                            ...
                                            return qrsInteract.Get(subjectAreaPath)
                                                .then(function (subjectAreas) {
                                                    logger.debug('subjectAreas:' + JSON.stringify(subjectAreas.body[0].choiceValues), {
                                                        module: 'doWork'
                                                    });
                                                    ...
                                                    logger.debug('Getting list of apps that have ' + config.gms.customPropName + ' applied', {
                                                        module: 'doWork',
                                                        method: 'updateAll'
                                                    });
                                                    return qrsInteract.Get(appCheckPath)
                                                        .then(function (appRefList) {
                                                            return Promise.each(appRefList.body, function (appRef) {
                                                                ...
                                                                logger.info('Updating app ' + appRef.name, {
                                                                    method: 'doWork'
                                                                });
                                                                logger.info('Fetching the fields of ' + appRef.id, {
                                                                    module: 'doWork',
                                                                    method: 'updateAll',
                                                                    app: appRef.name
                                                                });
                                                                ...
                                                                return ((config.gms.fieldCheck) ? getAppMetadata(appRef.id) : Promise.resolve())
                                                                    .then(function (appMetadata) {
                                                                        ...
                                                                        logger.info('Updating app:' + appRef.name + ' with id:' + appRef.id + 'currently owned by ' +
                                                                            appRef.owner.userDirectory + '\\' + appRef.owner.userId, {
                                                                                module: 'doWork',
                                                                                method: 'updateAll',
                                                                                app: appRef.name
                                                                            });
                                                                        ...
                                                                        return updateMetrics.updateMetrics(appRef, updateMatrix)
                                                                            .then(function (outcome) {
                                                                                ...
                                                                                logger.info('' + outcome.result + '::' + appRef.name, {
                                                                                    module: 'doWork',
                                                                                    method: 'updateAll',
                                                                                    app: appRef.name
                                                                                });
                                                                                var endTime = new Date();
                                                                                logger.info('SUCCESS: Process took ' + (endTime.getTime() - startTime.getTime()) + " milliseconds", {
                                                                                    module: 'doWork',
                                                                                    method: 'updateAll',
                                                                                    app: appRef.name
                                                                                })
                                                ...
                                                .catch(function (error) {
                                                    logger.error('getSubjectAreas::' + error, {
                                                        module: 'doWork',
                                                        method: 'updateAll'
                                                    });
                                                    reject(error);
                                        ...
                                        .catch(function (error) {
                                            logger.error('getMetricsTable::' + error, {
                                                module: 'doWork',
                                                method: 'updateAll'
                                            });
                                            reject(error);
                                ...
                                if (customProp.body == undefined || customProp.body.length == 0) {
                                    ...
                                    return gethypercube.getMetricsTable()
                                        ...
                                        .then(function (matrix) {
                                            ...
                                            if (body.mode.toUpperCase() == "SINGLE" && body.hasOwnProperty("appId")) {
                                                appCheckPath = "/app/full?filter=id eq " + body.appId;
                                                logger.info("Retrieving app info for " + body.appId, {
                                                    module: 'doWork',
                                                    method: 'updateAll'
                                                });
                                            } else if (body.mode.toUpperCase() == "SINGLE" && body.hasOwnProperty("appName")) {
                                                appCheckPath = "/app/full?filter=name eq " + body.appName;
                                                logger.info("Retrieving app info for " + body.appName, {
                                                    module: 'doWork',
                                                    method: 'updateAll'
                                                });
                                            } else {
    ...
    reloadMetricsApp: function () {
        return new Promise(function (resolve, reject) {
            ...
            return getdoc.getAppReference(config.gms.appName)
                .then(function (app) {
                    x.app = app;
                    logger.debug('reloadMetricsApp::reloadMetrics', {
                        module: 'doWork'
                    });
                    return reloadMetrics.reloadMetrics(x.app, config.gms.taskName)
                        .then(function (response) {
                            logger.debug('reloadMetricsApp success::' + response, {
                                module: 'doWork'
                            });
                            var res = {
                                complete: true,
                        ...
                        .catch(function (error) {
                            logger.error('reloadMetricsApp::reloadMetrics::' + error, {
                                module: 'doWork'
                            });
                            reject(new Error(error));
                        });
                ...
                .catch(function (error) {
                    logger.error('reloadMetricsApp::getAppReference::' + JSON.stringify(error), {
                        module: 'doWork'
                    });
                });
        });
            ...
            qrsInteract.Get(appCheckPath)
                ...
                .then(function (appArray) {
                    Promise.map(appArray, function (app) {
                        ...
                        .catch(function (error) {
                            logger.error('getAllMdi failure::' + error, {
                                module: 'doWork'
                            });
                            reject(error);
                        });
                ...
                .catch(function (error) {
                    logger.error('qrsinteract failure::' + error, {
                        module: 'doWork'
                    });
                    reject(error);
                })
