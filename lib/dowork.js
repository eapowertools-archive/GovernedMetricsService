...
var doWork = {
    ...
    getAllMdi: function() {
        return new Promise(function(resolve, reject) {
            ...
            var appCheckPath = "/app/full"
            appCheckPath += "?filter=customProperties.definition.name eq '";
            appCheckPath += config.gms.masterLibraryInputPropName + "' and customProperties.value ne null";
                ...
                .then(function(appList) {
                    return appList.map(function(app) {
                        return {
                            "id": app.id,
                            "name": app.name,
                            "subjectArea": getAppMasterLibrarySource(app, masterLibrarySourcePropName)
                        };
                    })
                })
