            ...
            qrsInteract.Get(path)
                .then(function(appObjects) {
                    if (appObjects.body.length !== 0) {
                        ...
                        logger.info(appInfo.name + " is not a published app, therefore, no metrics will be published until the app is published.  Metrics should appear in unpublished apps.", { module: "publishMetrics", app: appInfo.name });
                        resolve(appInfo.name + " is not a published app, therefore, no metrics will be published until the app is published.  Metrics should appear in unpublished apps.");
                    }
                })
