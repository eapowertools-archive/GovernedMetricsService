...
var publishMetrics = {
    publishMetrics: function (appInfo) {
        return new Promise(function (resolve, reject) {
            logger.info("Begin publish process", {
                module: "publishMetrics",
                app: appInfo.name
            });
            var path = "/app/object";
            ...
            logger.info("Searching published apps for dimensions and measures with the tag 'gms'", {
                module: "publishMetrics",
                app: appInfo.name
            });
            qrsInteract.Get(path)
                .then(function (appObjects) {
                    if (appObjects.body.length !== 0) {
                        ...
                        logger.info("GMS does not need to publish metrics because " + appInfo.name + " is not a published app.  Metrics will appear in unpublished apps.", {
                            module: "publishMetrics",
                            app: appInfo.name
                        });
                ...
                .then(function (arrObjects) {
                    ...
                    return Promise.all(arrObjects.map(function (appObjectId) {
                            ...
                            logger.info("Publishing appObject " + appObjectId, {
                                module: "publishMetrics",
                                app: appInfo.name
                            });
                            var body = "";
                            return qrsInteract.Put(putPath, body)
                                .then(function (sCode) {
                                    if (sCode.statusCode == 200) {
                                        ...
                                        logger.info("Published appObject: " + appObjectId, {
                                            module: "publishMetrics",
                                            app: appInfo.name
                                        });
                                        return 1;
                                        ...
                                        logger.info("Failed to publish appObject: " + appObjectId, {
                                            module: "publishMetrics",
                                            app: appInfo.name
                                        });
                                        return 0;
                        ...
                        .then(function (resultArray) {
                            ...
                            logger.info("Published " + successCount + " of " + resultArray.length + "App Objects", {
                                module: "publishMetrics",
                                app: appInfo.name
                            });
                ...
                .catch(function (error) {
                    logger.error("Error in publishMetrics: " + JSON.stringify(error), {
                        module: "publishMetrics",
                        app: appInfo.name
                    });
                    reject(error);
