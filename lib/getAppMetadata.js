...
function getAppMetadata(appId) {
    logger.info('running getAppMetadata', {
        module: 'getAppFields'
    });
    ...
    return session.open(qConfig)
        ...
        .then(function (layouts) {
            return session.close()
                .then(function () {
                    logger.info('Returning a list of fields of length ' + layouts.fields.qFieldList.qItems.length, {
                        module: 'getAppMetadata'
                    });
                    logger.info('Returning a list of variables of length ' + layouts.variables.qVariableList.qItems.length, {
                        module: 'getAppMetadata'
                    });
        ...
        .catch(function (err) {
            return session.close()
                .then(function () {
                    logger.error('getAppFields:: ' + JSON.stringify(err), {
                        module: 'getAppFields'
                    });
                })
        });
