    ...
    return new Promise(function(resolve, reject) {
        ...
        if (fs.existsSync(updateHandleFile)) {
            ...
            qrsNotify.deleteNotification(handle[0])
                .then(function(result) {
                    if (result == 204) {
                        logger.info("Update notification handle removed", { module: 'server' });
                    }
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (fs.existsSync(deleteHandleFile)) {
            ...
            qrsNotify.deleteNotification(handle[0])
                .then(function(result) {
                    if (result == 204) {
                        logger.info("Delete notification handle removed", { module: 'server' });
                    }
...
function createDeleteNotification(handleFile) {
    return new Promise(function(resolve, reject) {
        logger.info("Creating new notification handler for GMS based deletions to the repository.", { module: 'server' });
