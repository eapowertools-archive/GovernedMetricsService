    ...
    updateRepo: function (notifyObjects) {
        return new Promise(function (resolve, reject) {
            ...
            return Promise.all(objects.map(function (object) {
                        ...
                        return createBody(filteredItems)
                            .then(function (body) {
                                return changeOwner.changeAppObjectOwner(body, appInfo)
                                    .then(function () {
                                        socketHelper.logMessage("debug", "gms", "Attempting app object publishing on " + appInfo.name, __filename);
                                        return publishMetrics.publishMetrics(appInfo);
                                    })
                                    .catch(function (error) {
                                        socketHelper.logMessage("error", "gms", "Failed on change ownership call: " + JSON.stringify(error), __filename);
                                        reject(error);
                                    })
                            })
                    }));
                ...
                .then(function () {
                    socketHelper.logMessage("info", "gms", "Ownership change on notified app objects", __filename);
                    socketHelper.logMessage("info", "gms", "Please note that metrics may not appear immediately.  Recommendations include refreshing browser, browser session, and restarting the engine.", __filename);
