    ...
    updateRepo: function (notifyObjects) {
        return new Promise(function (resolve, reject) {
            ...
            return Promise.all(objects.map(function (object) {
                        ...
                        return createBody(filteredItems)
                            .then(function (body) {
                                return changeOwner.changeAppObjectOwner(body, appInfo)
                                    .then(function () {
                                        logger.info("Attempting app object publishing on " + appInfo.name, {
                                            module: "notifiedByRepo",
                                            method: "updateRepo"
                                        });
                                        return publishMetrics.publishMetrics(appInfo);
                                    })
                                    .catch(function (error) {
                                        logger.error("Failed on change ownership call", {
                                            module: "notifiedByRepo",
                                            method: "updateRepo"
                                        });
                ...
                .then(function () {
                    logger.info("Changed ownership on notified app objects", {
                        module: "notifiedByRepo",
                        method: "updateRepo"
                    });
                    logger.info("Please note that metrics may not appear immediately.  Recommendations include refreshing browser, browser session, and restarting the engine.")
                    resolve("Changed ownership on notified app objects");
                })
                .catch(function (error) {
                    logger.error("failed after Promise All", {
                        module: "notifiedByRepo",
                        method: "updateRepo"
                    });
                    logger.error(JSON.stringify(error), {
