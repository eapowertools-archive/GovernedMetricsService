...
var qrsChangeOwner = {
    changeAgent: function() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            var path = "/app/object";
            path += "?filter=owner.userId eq '" + config.repoAccountUserId + "' and owner.userDirectory eq '";
            path += config.repoAccountUserDirectory + "' and (objectType eq 'dimension' or objectType eq 'measure')";
        ...
        return new Promise(function(resolve, reject) {
            ...
            sequence = sequence
                ...
                .then(function(objInfo) {
                    ...
                    ////logger.debug('these items are currently owned by ', item.owner.userDirectory ,'\\', config.repoAccountUserDirectory, {module:'qrsChangeOwner',method:'processAppObjects'});
        ...
        uniqueApps.forEach(function(item, index, array) 
        {
            ...
            //logger.debug(appCount, ' of ', array.length,  {module: 'qrsChangeOwner', method: 'segregateAppObjects'});
                ...
                .then(function(result)
                {
                    ...
                    if(x.owner.userId != config.repoAccountUserId)
                    {
