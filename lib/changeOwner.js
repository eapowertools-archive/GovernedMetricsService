...
var changeOwner = {
    changeAppObjectOwner: function (body, appInfo) {
        return new Promise(function (resolve, reject) {
            logger.info("Starting App Object Ownership Change.", {
                module: "changeOwner",
                app: appInfo.name,
                owner: appInfo.ownerId
            });
            var x = {};
            var postPath = "/selection";
            return qrsInteract.Post(postPath, body, 'json')
                .then(function (selection) {
                    logger.debug("selection id = " + selection.body.id, {
                        module: "changeOwner",
                        app: appInfo.name
                    });
                ...
                .then(function () {
                    ...
                    return qrsInteract.Put(putPath, body)
                        ...
                        .catch(function (error) {
                            logger.error("Failed on put of ownership change objects", {
                                module: "changeOwner",
                                app: appInfo.name
                            });
                            reject(error);
                ...
                .then(function (sCode) {
                    logger.info("The response code from the ownership put: " + sCode, {
                        module: "changeOwner",
                        app: appInfo.name
                    });
                    if (sCode == 204) {
                        logger.info("Processed Change Ownership Request", {
                            module: "changeOwner",
                            app: appInfo.name
                        });
                        ...
                        return qrsInteract.Delete(deletePath)
                            .then(function (result) {
                                logger.info("deleted selection with id: " + x.selectionId, {
                                    module: "changeOwner",
                                    app: appInfo.name
                                });
                                logger.info("Change Ownership Complete", {
                                    module: "changeOwner",
                                    app: appInfo.name
                                });
                            ...
                            .catch(function (error) {
                                logger.error("Failed on delete call of the selection during ownership change", {
                                    module: "changeOwner",
                                    app: appInfo.name
                                });
                                reject(error);
                ...
                .catch(function (error) {
                    logger.error("Failed on Post to get selection id", {
                        module: "changeOwner",
                        app: appInfo.name
                    });
                    reject(error);
