...
var createMeasure = {
            ...
            .then(function (meas) {
                return meas.getProperties()
                    .then(function (currentProps) {
                        logger.debug("Measure: " + objId + " exists.  Checking for changes.", {
                            module: "createMeasure",
                            app: appRef.name
                        });
                        if (JSON.stringify(currentProps) == JSON.stringify(object)) {
                            logger.debug("Measure: " + objId + " no changes found.", {
                                module: "createMeasure",
                                app: appRef.name
                            });
                            return "SAME";
                        } else {
                            logger.debug("Measure: " + objId + " found changes.  Setting properties.", {
                                module: "createMeasure",
                                app: appRef.name
                            });
                            return meas.setProperties(object)
                                .then(function () {
                                    logger.debug("Measure: " + objId + " new properties set.", {
                                        module: "createMeasure",
                                        app: appRef.name
                                    });
    ...
    createMeasure: function (appRef, app, data, tags) {
        ...
        logger.debug("Measure: " + objId + " does not exist.  Creating", {
            module: "createMeasure",
            app: appRef.name
        });
        return app.createMeasure(object)
            .then(function (newMeas) {
                logger.debug("Measure: " + objId + " Created", {
                    module: "createMeasure",
                    app: appRef.name
                });
...
function meas(data, tags) {
    ...
    if (data[10].qText !== undefined) {
        if (data[10].qText !== '' || data[10].qText !== null || data[10].qText !== '-') {
            meas.qMeasure.coloring.baseColor = {
                color: data[10].qText,
                index: -1
            };
        }
